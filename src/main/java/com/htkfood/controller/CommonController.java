...
public class CommonController {
	...
	@Privileges(PrivilegeConstant.OPEN_PAGE)
	@RequestMapping(value ="/getDriveies", method = RequestMethod.POST)
    public Map<String,Object> getDriveies(@RequestBody SearchVo model,HttpServletRequest request) throws CommonException  {
		UserEntity user=(UserEntity) request.getSession().getAttribute("user");
		Map<String, Object>result=ResultHelper.success();
		model.setOrganization((String) request.getSession().getAttribute("queryOrg"));
		commonService.getDriverByUser(user, model,result);
        return result;
    }
    ...
    public Map<String,Object> getVehicles(@RequestBody SearchVo model,HttpServletRequest request) throws CommonException  {
		UserEntity user=(UserEntity) request.getSession().getAttribute("user");
		Map<String, Object>result=ResultHelper.success();
		model.setOrganization((String) request.getSession().getAttribute("queryOrg"));
		commonService.getVehicleByUser(user, model,result);
        return result;
    }
    ...
    public void exportReport(@RequestParam(required=false)String expressCompany,@RequestParam(required=false)Integer timeType,
    		@RequestParam(required=false)Long beginTime,
    		@RequestParam(required=false)Long endTime,HttpServletRequest request,HttpServletResponse response) throws CommonException, IOException  {
		UserEntity user=(UserEntity) request.getSession().getAttribute("user");
...
//		model.setOrganization(org);
		model.setBeginTime(beginTime);
		model.setEndTime(endTime);
		HSSFWorkbook wb =commonService.exportOrderReport(user, model);
		response.setContentType("application/vnd.ms-excel");  
